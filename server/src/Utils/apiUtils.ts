...
export const getMonitoredUpInfo = async (id: number) => {
    let res = await LiveServer.findAll({
      ...
      let totalUptime = res.reduce((acc: any, curr: any, ind: number) => {
        if(curr.status === 'up' && res[ind + 1] && res[ind + 1].status === 'down') {
          let difference = dayjs(res[ind + 1].time).diff(dayjs(curr.time), 'seconds');
      ...
      let totalDowntime = res.reduce((acc: any, curr: any, ind: number) => {
        if(curr.status === 'down' && res[ind + 1] && res[ind + 1].status === 'up') {
          let difference = dayjs(res[ind + 1].time).diff(dayjs(curr.time), 'seconds');
          return acc + difference;
      ...
      let getTotalMonitoringTime = dayjs(Date.now()).diff(dayjs(res[0].time), 'seconds');
      let percentageUp = Number(((totalUptime / getTotalMonitoringTime) * 100).toFixed(2));
      let percentageDown = Number(((totalDowntime / getTotalMonitoringTime) * 100).toFixed(2));
